  ...
  <div class="cart">
      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="(item) in cartInfoList" :key="item.id">
          <li class="cart-list-con1">
            <input type="checkbox" name="chk_list" :checked="item.isChecked" @change="updata(item,$event)">
          </li>
          ...
          <li class="cart-list-con5">
            ...
            <input autocomplete="off" type="number" minnum="1" class="itxt" :value="item.skuNum" @change="handler('change',$event.target.value*1,item)">
          ...
          <li class="cart-list-con7">
            <a  class="sindelet" @click="deleteshop(item)">删除</a>
            <br>
            <a href="#none">移到收藏</a>
          </li>
        </ul>
    ...
    <div class="cart-tool">
      <div class="select-all">
        <input :checked="checkedall" class="chooseAll" type="checkbox" @change="upchecked">
        <span>全选</span>
      </div>
      <div class="option">
...
export default {
  ...
  methods: {
    ...
    handler: throttle(async function (type, disNum, cart) { 
      ...
      try {
        await this.$store.dispatch('addshop', { skuId: cart.skuId, skuNum: disNum })
        this.getdata()
      } catch (error) {
  ...
  async  updata(cart, event) {
     try {
       let isChecked = event.target.checked ? '1' : '0'
       console.log(isChecked)
       await this.$store.dispatch('updata', { skuId: cart.skuId, isChecked:isChecked })
       this.getdata()
     } catch (error) {
      alert(error.message)
     }
    ...
    async deleteall() { 
     try {
       await this.$store.dispatch('deletAll')
      this.getdata()
     } catch (error) {
      alert(error.message)
     }
    },
   async upchecked(event) { 
      try {
        let isChecked = event.target.checked ? '1' : '0'
