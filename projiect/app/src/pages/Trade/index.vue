  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(address) in addressinfo" :key="address.id">
        ...
        <p @click="changeDeafult(address,addressinfo)">
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="(shop,index) in orderinfo.detailArrayList" :key="index">
          <li>
            <img :src="shop.imgUrl" alt="" style="width: 82px; height: 82px;">
          </li>
          <li>
            <p>
              {{ shop.skuName }}</p>
    ...
    <div class="sub clearFix">
      <button class="subBtn" @click="getpay">提交订单</button>
    </div>
  </div>
</template>
...
import { mapState, mapGetters } from 'vuex'
  ...
  data() {
    ...
    mounted() {
      this.$store.dispatch('getuserlisst')
      this.$store.dispatch('getshopinfo')
  },
  computed: {
    ...mapState({
      addressinfo: state => state.trade.peoples,
  ...
  methods: {
    changeDeafult(address, addressinfo) { 
    ...
    async getpay() { 
      ...
      const result = await this.$API.reqsubmit(tradeNo, data)
      console.log(result)
      if (result.code === 200) {
        this.shopid = result.data
        this.$router.push(`/pay?orderId=${this.shopid}`)
      } else { 
        alert('订单获取失败')
      }
    }
