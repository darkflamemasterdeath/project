          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(spu) in spuSaleAttrList" :key="spu.id">
                ...
                <dd changepirce="0" :class="{active:sp.isChecked==1}" 
                v-for="(sp) in spu.spuSaleAttrValueList" 
                :key="sp.id" @click="changeActive(sp, spu.spuSaleAttrValueList)">{{sp.saleAttrValueName  }}</dd>
              </dl>
              <dl>
...
return this.skuInfo.skuImageList || []
  ...
  methods: {
    ...
    changenum(event) {
      ...
      async addshop() { 
      try {
        await this.$store.dispatch('addshop', { skuId: this.$route.params.skuId, skuNum: this.num })
